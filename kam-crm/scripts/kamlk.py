...
DEFAULT_SOURCE_TAB = "Updated KAM List"
...
def ensure_supabase_schema(engine) -> None:
    ...
    CREATE TABLE IF NOT EXISTS {SUPABASE_SCHEMA}.{SUPABASE_DOD_TABLE} (
        ...
        {', '.join(f'day_{n:02d} bigint NOT NULL DEFAULT 0' for n in range(1, 31))},
...
def build_supabase_dod_rows(
    ...
    if len(day_columns) != 30:
        raise ValueError("Supabase KAM DOD sync requires exactly 30 day columns.")
    rows: List[dict] = []
    for _, row in dod_report.iterrows():
        ...
        for day_number, source_column in enumerate(day_columns, start=1):
            record[f"day_{day_number:02d}"] = int(row[source_column])
        rows.append(record)
    return rows
...
def sync_supabase_current_state(
    ...
    dod_columns = [f"day_{day_number:02d}" for day_number in range(1, 31)]
    dod_column_sql = ", ".join(dod_columns)
    dod_value_sql = ", ".join(f":{column}" for column in dod_columns)
    dod_update_sql = ",\n            ".join(
        f"{column} = EXCLUDED.{column}" for column in dod_columns
    )
    dod_upsert = text(
        f"""
        INSERT INTO {SUPABASE_SCHEMA}.{SUPABASE_DOD_TABLE} (
            business_id, business_name, kam_name, onboarded_date, report_month,
            {dod_column_sql}, active_days, refreshed_at
        ) VALUES (
            :business_id, :business_name, :kam_name, :onboarded_date,
            :report_month, {dod_value_sql}, :active_days, now()
        )
        ON CONFLICT (business_id) DO UPDATE SET
            business_name = EXCLUDED.business_name,
            kam_name = EXCLUDED.kam_name,
...
def read_kam_list(workbook, tab_name: str) -> pd.DataFrame:
    ...
    if header_row is None:
        raise ValueError(
            ...
            f"'{tab_name}'. Expected ID/id (or Business ID), Business Name, "
            f"and KAM (or KAM Name)."
        )
    print(f"KAM source header row detected: {header_row}")
    source_columns = {
    ...
    if len(duplicate_ids):
        print(
            "WARNING: duplicate business IDs found in Updated KAM List; "
...
def build_dod_report(
    ...
    """
    Build KAM DOD with the exact requested layout:
      A Business ID
      B Business Name
      C KAM name
      D Onboarded date
      E:AH 30 business-day order columns
      AI Active Days
    """
    day_list = [
    ...
    if len(day_list) != 30:
        raise ValueError(
            "KAM DOD must contain exactly 30 business dates so the daily "
            "values occupy E:AH and Active Days remains in AI."
        )
    if counts_df.empty:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "First KAM DOD business date. Default: first day of the cutoff "
            "month."
        ),
    )
    parser.add_argument(
        ...
        help=(
            "Last KAM DOD business date. Default: 29 days after DOD start, "
            "keeping the requested 30 daily columns."
        ),
    )
    parser.add_argument(
        "--skip-supabase",
...
def main() -> None:
    ...
    dod_start_date = args.dod_start_date or cutoff_date.replace(day=1)
    dod_end_date = args.dod_end_date or (dod_start_date + timedelta(days=29))
    ...
    if (dod_end_date - dod_start_date).days != 29:
        raise ValueError(
            "KAM DOD requires exactly 30 dates so E:AH contains daily values "
            "and AI contains Active Days."
        )
    if dod_start_date.day != 1:
        raise ValueError(
            "--dod-start-date must be the first day of the current reporting "
            "month so Retention & Target can calculate the full previous month."
        )
    report_date = cutoff_date - timedelta(days=1)
    previous_date = cutoff_date - timedelta(days=2)
    last_7_start = cutoff_date - timedelta(days=7)
    ...
    if not args.skip_supabase:
        ...
        # Alert generation MUST run before the sync overwrites yesterday's
        # button flags: it snapshots every still-unworked active button for
        # the previous reporting date into kam_alerts.
    ...
    if supabase_engine is not None:
        ...
        dod_rows = build_supabase_dod_rows(dod_report, current_month_start)
