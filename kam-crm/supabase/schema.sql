...
BEGIN
    ...
    -- Legacy KAM DOD redesign: the old wide day_01..day_30 shape cannot
    -- represent 28/29/31-day months or real date labels. kam_dod is fully
    -- rebuilt by every nightly run, so dropping the legacy shape is safe.
    IF EXISTS (SELECT 1 FROM information_schema.columns
...
-- ----------------------------------------------------------------------------
-- 1b. KAM DOD (current month, jsonb day values keyed by real dates) plus the
--     permanent month-by-month archive and the promised-order table.
-- ----------------------------------------------------------------------------
...
-- Upserted on every run keyed by (business_id, report_month); never deleted,
-- so each finished month's final state is stored the moment the next begins.
...
-- Promised order per merchant, entered by KAM/Lead/Admin in the web app.
